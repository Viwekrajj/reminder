...
function App() {
  ...
  const getReminders = async () => {
    const response = await axios.get("/reminder").catch((err) => {
  ...
  const addReminder = async (event) => {
    ...
    const response = await axios.post("/reminder", formData).catch((err) => {
      console.log("Error: ", err);
    });
  ...
  const deleteReminder = async (id) => {
    const response = await axios.delete(`/reminder/${id}`).catch((err) => {
